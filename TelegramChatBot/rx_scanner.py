...
def connect():
    ...
    try:
        conn = mysql.connector.connect(host='localhost',database='autotester',user='tableau', password='Tthht@123')
...
def list_session2(cond):
    sessions=[]
    try:
        conn = connect()
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM session where swname like '{cond}'")
        sessions = cursor.fetchall()
    except Error as e:
        print(e)
    finally:
...
def rxscan(swname):
    print("rxscan a switch")
    sessions = list_session2(swname)
    if (len(sessions) < 1):
        return False
    session_id=sessions[0][0]
    ip_add=sessions[0][1]
    username=sessions[0][2]
    password=sessions[0][3]
    try:
        cmds = list_command(session_id)
        #print(cmds)
        active_session = telnet(ip_add,username,password)
